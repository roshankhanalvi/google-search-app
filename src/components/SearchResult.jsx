...
const SearchResult = () => {
    ...
    const fetchSearchResult = () => {
        ...
        fetchDataFromApi(payload).then((data)=>{
            console.log(data)
            setResult(data)
        })
    }
    ...
    return <div className="flex flex-col min-h-[100vh]">
        ...
        <main className="grow p-[12px] pb-0 md::pr-5 md:pl-20">
            ...
            {!imageSearch ? (<>{items.map((item,index)=>(
                <SearchedItemTemplate key={index} data={item} />
            ) )}</>)
             : (<div className="grid grid-cols-2 md:grid-cols-4 xl:grid-cols-6 gap-4"> {items.map((item,index)=>(
